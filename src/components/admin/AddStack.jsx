...
import { addStack } from "../../context/actions/stackAction";
export default function AddStack() {
  const { dispatch: alertDispatch } = useContext(AlertContext);
  const {
    dispatch: stackDispatch,
    state: { addStackLoading },
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        addStack(stackDispatch, alertDispatch, uiDispatch, inputs, setInputs);
      }}
    ...
    >
      ...
      <div className="w-full flex flex-row justify-center items-center gap-5">
        ...
        >
          {addStackLoading ? <SpinnerLoading size={`30px`} /> : "Publish"}
        </button>
      </div>
    </form>
  );
